...
Browsertime.prototype._do = function(options, cb) {
  ...
  async.series(browserRuns,
    function(err, results) {
      if (err) {
        self.log.error('Error running browser: ' + util.inspect(err));
        return cb(err);
      }
...
Browsertime.prototype._getFormattedResult = function(options) {
  ...
  var browsertimeData = this.result[0].browsertime;
  return {
    url: options.url,
    runs: options.runs,
    ...
    platform: this.os,
    userAgent: browsertimeData.userAgent,
    windowSize: browsertimeData.windowSize,
